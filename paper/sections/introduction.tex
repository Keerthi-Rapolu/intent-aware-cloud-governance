...
PBCP is built around two complementary metrics. The first is \emph{Cost Prevention Score} (CPS), which measures how much waste was prevented before billing. The second is \emph{Intent-Fit Score} (IFS), which measures how well observed runtime behavior aligned with declared workload intent. Together with Execution Success Rate (ESR), these metrics separate prevention value from system aggressiveness and make system-level evaluation more defensible.
...
\begin{itemize}
  ...
  \item An IFS-based anomaly-detection method for identifying intent-behavior divergence earlier than utilization-only baselines.
\end{itemize}
