"""
Fig 1 — Simulation Calibration (Exp 0)
...
def make_figure(df: pd.DataFrame) -> plt.Figure:
    ...
    # --- Panel A: utilization scatter ---
    ax = axes[0]
    for wtype, grp in df.groupby("workload_type"):
        c = WORKLOAD_COLORS.get(wtype, "#7f7f7f")
        ax.scatter(grp["actual_util"], grp["predicted_util"],
                   color=c, label=wtype, s=28, alpha=0.75, edgecolors="none")
    ...
    ax.annotate(f"MAE = {mae:.4f}", xy=(0.05, 0.92), xycoords="axes fraction",
                fontsize=9, color="#333333")

    # --- Panel B: cost scatter ---
    ax = axes[1]
    for wtype, grp in df.groupby("workload_type"):
        c = WORKLOAD_COLORS.get(wtype, "#7f7f7f")
        ax.scatter(grp["actual_pot_cost"], grp["predicted_pot_cost"],
                   color=c, label=wtype, s=28, alpha=0.75, edgecolors="none")
    ...
    rel_rmse = np.sqrt(
        ((df["predicted_pot_cost"] - df["actual_pot_cost"]) ** 2).mean()
    ) / df["actual_pot_cost"].mean()
    ...
    ax.annotate(f"rel-RMSE = {rel_rmse:.4f}", xy=(0.05, 0.92), xycoords="axes fraction",
                fontsize=9, color="#333333")
    ...
    handles = [
        ...
        for wt, c in WORKLOAD_COLORS.items()
    ]
    handles.append(
        mlines.Line2D([], [], color="black", linestyle="--", linewidth=1.0, label="y = x")
    )
    fig.legend(handles=handles, loc="lower center", ncol=7,
               fontsize=8, bbox_to_anchor=(0.5, -0.06))

    fig.suptitle("Figure 1 — Simulation Calibration (Exp 0)", fontsize=11, y=1.01)
    fig.tight_layout()
    return fig
